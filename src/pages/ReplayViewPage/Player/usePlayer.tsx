...
const usePlayer = function () {
  ...
  const {
    ...
    isFullScreen,
    captionLang,
  } = useSelector(({ replayView, player }: RootState) => ({
  ...
  useEffect(() => {
    if (replayViewInfo.captions && captionLang && replayViewInfo.captions[captionLang]) {
      if (captionObj === null || !captionObj[captionLang]) {
        axios
          .get(replayViewInfo.captions[captionLang])
          .then((res) => {
            if (res.status === 200) {
  ...
  }, [replayViewInfo.captions, captionLang]);
  ...
  useEffect(() => {
    ...
    setUrl(replayViewInfo.vodUrls[quality]);
  }, [replayViewInfo.vodUrls, quality]);
